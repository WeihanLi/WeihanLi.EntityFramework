...
namespace WeihanLi.EntityFramework
{
    public static class QueryablePageListExtensions
    {
        ...
        /// <summary>
        /// Converts the specified source to <see cref="IListResultWithTotal{T}"/> by the specified <paramref name="pageNumber"/> and <paramref name="pageSize"/>.
        /// </summary>
        /// <typeparam name="T">The type of the source.</typeparam>
        /// <param name="source">The source to paging.</param>
        /// <param name="pageNumber">The number of the page, index from 1.</param>
        ...
        ///     A <see cref="CancellationToken" /> to observe while waiting for the task to complete.
        /// </param>
        /// <returns>An instance of  implements  <see cref="IListResultWithTotal{T}"/> interface.</returns>
        public static async Task<IListResultWithTotal<T>> ToListResultWithTotalAsync<T>([NotNull] this IQueryable<T> source, int pageNumber, int pageSize, CancellationToken cancellationToken = default)
        {
            if (pageNumber <= 0)
            {
                pageNumber = 1;
